...
def test_load_targets_and_annotations(tmp_path):
...
def test_load_screen_pipeline(tmp_path):
    mmc2 = _make_mmc2(tmp_path)
    targets = load_targets(mmc2)
    annots = load_annotations(mmc2)
    ...
    assert any(r.fold_change < 0 for r in records)  # sign preserved -- hit-calling is sign-dependent

    r = next(r for r in records if r.guide_id == "gL_000001")
    assert r.target == "Hum_GENE1" and r.target_sequence == "AAACCCGGGTTT"
    assert r.chrom == "1" and r.strand == "+" and r.closest_pc_gene == "GENE_A"
    assert r.distance_to_closest_pc_gene == 18435

    no_annot = load_screen(_make_mmc3(tmp_path), targets, annotations={})[0]
    assert no_annot.chrom == "" and no_annot.distance_to_closest_pc_gene is None


def test_to_dataframe_jsonl_and_from_dict(tmp_path):
    mmc2 = _make_mmc2(tmp_path)
    targets = load_targets(mmc2)
    annots = load_annotations(mmc2)
    records = load_screen(_make_mmc3(tmp_path), targets, annotations=annots)
    ...
    r2 = ScreenRecord.from_dict(base)  # optional fields omitted entirely
