...
class TestBuildFeatures:
    ...
    def test_day_onehot_columns_present(self):
        ...
        X, _ = build_features(records, k=3)
        assert "day_7" in X.columns
        assert "day_14" in X.columns
    ...
    def test_cell_line_onehot_columns_present(self):
        ...
        for cl in ["HAP1", "HEK293FT", "K562", "MDA-MB-231", "THP1"]:
            assert f"cell_{cl}" in X.columns

    def test_include_distance_true(self):
        records = [_rec("ACGTACGTACGTACGTACGTACG", distance=500)]
        X, _ = build_features(records, k=3, include_distance=True)
        assert "distance_to_closest_pc_gene" in X.columns
        assert X["distance_to_closest_pc_gene"].iloc[0] == 500.0

    def test_include_distance_none_becomes_sentinel(self):
        records = [_rec("ACGTACGTACGTACGTACGTACG", distance=None)]
        X, _ = build_features(records, k=3, include_distance=True)
        assert X["distance_to_closest_pc_gene"].iloc[0] == -1.0

    def test_include_distance_false(self):
        records = [_rec("ACGTACGTACGTACGTACGTACG", distance=500)]
        X, _ = build_features(records, k=3, include_distance=False)
        assert "distance_to_closest_pc_gene" not in X.columns
